...
impl<F: Field> Assignment<F> for Layout {
    ...
    fn get_challenge(&self, _: crate::dev::Challenge) -> Value<F> {
        Value::unknown()
    }
...
impl<G: PrimeGroup, ConcreteCircuit: Circuit<G::Scalar>> CircuitCost<G, ConcreteCircuit> {
    ...
    pub fn measure(k: u32, circuit: &ConcreteCircuit) -> Self {
        ...
        .unwrap();
        let (cs, _) = cs.compress_selectors(layout.selectors.clone());
        ...
        CircuitCost {
            ...
            constants: cs.constants.len(),
            gates: cs.gates.len(),
            regions: layout
                .regions
                .iter()
                .map(|r| (r.name.clone(), r.rows))
                .collect(),
        }
    }
